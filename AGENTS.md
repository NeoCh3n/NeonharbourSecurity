...
|-------|---------------|-------------------|------------------|---------------|
| Plan | **Planner** | EventBridge `AgenticAlert` event + Lambda Ingest | Normalise inbound Sentinel/Splunk/Defender/Okta signals, enrich with tenant metadata, persist to DB | Investigation envelope (`TENANT#*` partition) |
...
| Analyze | **Analyst** | Step Functions `SummarizeWithAI` task → Lambda Summarize + Bedrock | Run RAG-backed reasoning loop (BedrockAnalyst runnable), map findings to HKMA SA-2 / TM-G-1, return confidence, timeline, guard-railed actions | Structured summary JSON (Summary should be dynamic and AI provider tagged) |
| Respond | **Risk Orchestrator** | Step Functions `RiskDecider` Choice + `RequestApproval` | Compute MTTA/MTTI/MTTR/FPR deltas, classify risk, queue HITL approval (the plan should be dynamic based on the alert), enforce allow-list actions | Risk snapshot + approval request (S3 `approvals/…`) |
| Adapt | **Learning Curator** | Step Functions `AdaptInsights` task → Lambda Adapt | Capture feedback (risk outcome, actions, metrics) and call `AnalystLLM.record_feedback` for per-tenant tuning; persist adaptation hints in DB | Adaptation record (`adaptation` attribute) + JSONL audit |
| Report | **outcome** | Step Functions `WriteAuditTrail` task → Lambda Audit | Write immutable S3 JSONL, update DB status, emit metrics to CloudWatch dashboard + DB metrics table, if the alert is verified by all agent and not malicious, mark it False Positive and close it.

### Agent Contract Highlights
- **Shared Memory**: DB tables `NeoHarbor SecurityInvestigations-*` and `NeoHarbor SecurityMetrics-*` act as short-term shared memory with KMS encryption. Each agent writes stage-specific fields while respecting tenant partitions.
...
- **Registry & Messaging**: `src/agents/registry.py` persists agent metadata in DB, while `src/agents/messaging.py` publishes EventBridge telemetry for each stage.
...
- **Guardrails & Actions**: Allowed actions restricted to `ISOLATE_EC2`, `BLOCK_IP_WAF`, `DISABLE_KEYS`, `TICKET_UPSERT`. Additional actions require governance with human approvals logged to S3 Object Lock bucket.
- **Extensibility Roadmap**: introduces HITL automation (Teams/Slack approvals, ServiceNow/Jira tickets) and Detection Advisor cadence; extends integrations (email gateways, data lakes, threat intel) and SOC 2 readiness.

### AWS AI Hackathon Alignment
- **Infrastructure Footprint**: `infra/sam-template.yaml` codifies EventBridge → Lambda → Step Functions → DB/S3/KMS pipeline, CloudWatch dashboards, and least-privilege IAM policies mandated by HKMA.
- **GenAI Strategy**: Bedrock is runnable baseline (Claude 3 Haiku + Titan embeddings). 
- **Compliance Focus**: `docs/hkma/` templates and `Makefile compliance` target produce HKMA SA-2/TM-G-1 packs, encryption/retention policies, and Mermaid data-flow diagrams
...
- **KPIs & Auditability**: Metrics table captures MTTA/MTTI/MTTR/FPR with DB TTL for hygiene, CloudWatch dashboard surfaces state machine health, and audit bucket enforces WORM retention (7-year) via Object Lock.
